...
- `exo1`: lamp with one button and one LED; debounce uses an `xtimer_t` callback and atomic flags.
- `exo2`: code lock with four buttons and one status LED.
- `exo3`: ESP32 traffic light with pedestrian button.
- `exo4`: Morse code encoding/decoding with one-byte packed letter codes.
- `exo6`: non-standard integer workload; still needs official Dhrystone/CoreMark and is not considered complete.
- `exo7`: memory exploration in main, a second thread, and a timer callback.
...
- `exo16`: Modbus RTU slave on the console UART; builds successfully but still needs hardware validation.
...
`exo1` implements the lamp/switch task with interrupt handling, short press
to toggle blinking, and long press to change frequency. Debounce now uses an
`xtimer_t` callback. State shared with interrupt context is accessed through
`atomic_utils`, and elapsed-time subtraction remains valid across timer wrap.
...
`exo4` implements Morse code with GPIO. Each letter uses one byte: three bits
for length and five bits for dots/dashes. Characters are recovered from the
array index, while digit codes are generated by a small function. Button
debounce also uses an `xtimer_t` callback and atomic flags.

`exo7` now prints variables from the main thread and a second thread. A timer
callback captures its local/static values and addresses atomically, then the
main thread prints them because `printf` should not run inside interrupt context.
...
Frame completion now uses an `xtimer_t` callback after UART silence. Compilation
is verified, but the exercise still requires a physical Modbus test.
